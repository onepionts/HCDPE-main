...
class Classifer_loss(nn.Module):
    ...
    def forward(self, p_logits, p_joints, g_logits, joints):
        ...
        if self.token_loss_w > 0:
            ...
            losses_sum_token.append(token_loss)
        else:
            losses.append(None)
        ...
        if self.joint_loss_w > 0:
            ...
            joint_loss *= self.joint_loss_w
            losses.append(joint_loss)
            losses_sum_joints.append(joint_loss)
        else:
            losses.append(None)
        #print("pct_loss_88",losses_sum_token,sum(losses_sum_token),len(losses_sum_token),losses_sum_joints,sum(losses_sum_joints),len(losses_sum_joints))
        return losses