...
def main():
    ...
    output_dir = os.path.join(args.out_img_root) + video_name + '/'
    ...
    for ii in tqdm(range(video_length)):
        ...
        mmdet_results = inference_detector(det_model, image_name)#检测结果包含所有类别的边界框和置信度。

        # keep the person class bounding boxes.处理检测结果，只保留属于 args.det_cat_id 类别（即人）的边界框，得到 person_results
        ...
        pose_results, returned_outputs = inference_top_down_pose_model(
            pose_model,#当前帧中检测到的人进行姿态估计。
            image_name,
            person_results,
            bbox_thr=args.bbox_thr,#边界框置信度阈值，低于此值的边界框会被过滤掉。
            format='xyxy',
            dataset=dataset,
